(() => {
    ...
    function spollers() {
        ...
        if (spollersArray.length > 0) {
            ...
            function setSpollerAction(e) {
                ...
                if (el.closest("[data-spoller]")) {
                    ...
                    if (!spollersBlock.querySelectorAll("._slide").length) {
                        ...
                        spollerItem.classList.toggle("_spoller-open");
            ...
            function hideSpollersBody(spollersBlock) {
                ...
                const spollerActiveItem = spollersBlock.querySelector("[data-spoller]._spoller-open");
                const spollerSpeed = spollersBlock.dataset.spollersSpeed ? parseInt(spollersBlock.dataset.spollersSpeed) : 500;
                if (spollerActiveTitle && !spollersBlock.querySelectorAll("._slide").length) {
                    spollerActiveTitle.classList.remove("_spoller-active");
                    spollerActiveItem.classList.remove("_spoller-open");
            ...
            if (spollersClose.length) document.addEventListener("click", (function(e) {
                ...
                if (!el.closest("[data-spollers]")) spollersClose.forEach((spollerClose => {
                    ...
                    const spollerActiveItem = spollerClose.closest("[data-spoller-item]");
    ...
    let formValidate = {
        ...
        validateInput(formRequiredItem) {
            ...
            } else if ("amount" === formRequiredItem.dataset.required) {
                formRequiredItem.value = formRequiredItem.value.replace(" ", "");
                if (this.amountTest(formRequiredItem)) {
                    this.addError(formRequiredItem);
                    error++;
                } else {
                    this.removeError(formRequiredItem);
                    formRequiredItem.value = Number.parseFloat(formRequiredItem.value).toFixed(2);
                    formRequiredItem.value = "$ " + formRequiredItem.value;
                }
        ...
        phoneTest(formRequiredItem) {
            return !/^[\+]?[(]?[0-9]{3}[)]?[-\s\.]?[0-9]{3}[-\s\.]?[0-9]{4,6}$/.test(formRequiredItem.value);
        },
        amountTest(formRequiredItem) {
            return !/^\$?[0-9]\d*(((,\d{3}){1})?(\.\d{0,2})?)$/.test(formRequiredItem.value);
        }
    };
    function formSubmit() {
    ...
    function initSliders() {
        if (document.querySelector(".purchase__slider")) new core(".purchase__slider", {
            modules: [ Pagination ],
            observer: true,
            observeParents: true,
            slidesPerView: 1,
    ...
    da.init();
    const showMore = document.querySelector(".home-item-showmore__more");
    if (showMore) {
        const itemsLength = document.querySelectorAll(".home-item-showmore__item").length;
        let items = 0;
        let currentItem = 0;
        ...
        showMore.addEventListener("click", (() => {
            items += currentItem;
            const arrayItems = Array.from(document.querySelector(".home-item-showmore__content").children);
            const visibleItems = arrayItems.slice(0, items);
            visibleItems.forEach((el => el.classList.add("_visible")));
            if (visibleItems.length === itemsLength) showMore.style.display = "none";
        }));
    }
    window["FLS"] = true;
    isWebp();
